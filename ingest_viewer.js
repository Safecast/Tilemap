...
var IngestWords = (function()
{
    ...
    IngestWords.prototype.WordsFromNumber = function(number)
    {
    	// If the length isn't precisely 2048, this won't work
        if (IngestWords.Words2048.length != 2048) 
        {
		    return "not-2048";
	    }
	    ...
	    // Generate the string
	    return IngestWords.Words2048[left] + "-" + IngestWords.Words2048[middle] + "-" + IngestWords.Words2048[right];
    };

    // Look up a number from three simple words
    IngestWords.prototype.WordsToNumber = function(what) 
    {
	    var result = 0 >>> 0;
    ...
    IngestWords.prototype._WordToNumber = function(what)
    {
        var idx = 2049;
    
        for (var i=0; i<this.SortedWords.length; i++)
        {
            if (IngestWords.Words2048[this.SortedWords[i]] >= what)
            {
                idx = i;
                break;
            }//if
        ...
        if (idx < 2048 && IngestWords.Words2048[this.SortedWords[idx]] == what) 
        {
            return { ok:true, x:this.SortedWords[idx] };
        }//if
    
        return { ok:false, x:0 };
    };

    IngestWords.prototype._WordsInit = function()
    {
        // Init the index array
        this.SortedWords = new Uint32Array(2048);

        for (var i=0; i<2048; i++) 
        {
            this.SortedWords[i] = i;
        }//for

        // Sort the array
        this.SortedWords = _SortWords(this.SortedWords, IngestWords.Words2048);
    };
    ...
    IngestWords.Words2048 = 
    [
        "act","add","age","ago","aim","air","all","and","any","arm","art","ash","ask","bad","bag","ban","bar","bat","bay","bed","bee","beg","bet","bid","big","bit","bow","box","boy","bug","bus","buy","cab","can","cap","car","cat","cop","cow","cry","cue","cup","cut","dad","day","die","dig","dip","dog","dot","dry","due","ear","eat","egg","ego","end","era","etc","eye","fan","far","fat","fee","few","fit","fix",
...
[
  {
   ...
   var _GetDateTimeHtmlForDate = function(d)
    ...
    var _AddDeviceListToContainerForJsonIdx = function(json, idx, container, words)
    {
        if (json[idx].device_ids != null)
        {
            ...
            for (var i=0; i<IngestWords.Words2048.length; i++)
            {
                var w = words.WordsFromNumber(json[idx].device_ids[i]);
                maxwordslen = Math.max(w.length, maxwordslen);
            }//for
            ...
            for (var i=0; i<json[idx].device_ids.length; i++)
            {
                ...
                li.innerHTML = (json[idx].device_ids[i].toString().length >= maxlen ? "" : _GetNbspPaddingDigits(maxlen - json[idx].device_ids[i].toString().length))
                             ...
                             + "</a> "
                             + w
                             + (w.length >= maxwordslen ? "" : _GetNbspPaddingDigits(maxwordslen - w.length))
                             + " "
